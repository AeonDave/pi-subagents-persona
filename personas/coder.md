...
- Coach async: launch coaching-worthy or parallel legs `async: true` so you can peek, redirect, or stop them while they run, and a blocked child escalates over pi-intercom. Give parallel writers disjoint files (worktree isolation in a clean repo); you are the message bus.
